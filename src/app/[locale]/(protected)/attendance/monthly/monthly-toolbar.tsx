    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b p-2">
        <h1 className="font-bold tracking-tight">
          {t("attendanceMonthlyToolbarTitle")}
        </h1>
        ...
        <div className="flex items-center gap-2">
          {canManage && (
            <>
              <Button
                ...
                disabled={isPending}
              >
                <Calculator /> {t("attendanceMonthlyToolbarCalculate")}
              </Button>
              <Button
                variant="outline"
                size={"xs"}
                ...
                disabled={isPending}
              >
                <Lock /> {t("attendanceMonthlyToolbarLock")}
              </Button>
            </>
          )}
          <Button
          ...
          >
            <Download />
            {isExporting
              ? t("attendanceMonthlyToolbarExporting")
              : t("attendanceMonthlyToolbarExportExcel")}
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center px-2">
        <div className="flex items-center gap-1">
          <Button variant="outline" size="xs" onClick={onTodayClick}>
            {t("attendanceMonthlyToolbarToday")}
          </Button>
          <Button
            variant="outline"
            size="icon-xs"
          ...
          <span className="font-medium px-3 min-w-[120px] text-center">
            {t("attendanceMonthlyToolbarMonthYear", { month, year })}
          </span>
        </div>
        ...
        {departments.length > 0 && (
          ...
          >
            <SelectTrigger size="sm" className="w-[200px]">
              <SelectValue placeholder={t("attendanceMonthlyToolbarAllDepartments")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">
                {t("attendanceMonthlyToolbarAllDepartments")}
              </SelectItem>
              {departments.map((d) => (
        ...
        {viewMode === "grid" && (
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="show-weekend" className="text-sm cursor-pointer">
              {t("attendanceMonthlyToolbarShowWeekend")}
            </Label>
          </div>
        )}
        ...
        <div className="flex gap-1 ml-auto">
          ...
          >
            <CalendarDays className="h-4 w-4" /> {t("attendanceMonthlyToolbarGrid")}
          </Button>
          <Button
          ...
          >
            {t("attendanceMonthlyToolbarSummary")}
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs px-2">
        ...
        <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 bg-muted text-muted-foreground">
          <strong>—</strong> {t("attendanceMonthlyToolbarLegendWeekendFuture")}
        </span>
      </div>
    </div>
  );
