...
export function MonthlyClient({
  ...
  const calculateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("attendanceMonthlyClientCalculateSuccess"));
      queryClient.invalidateQueries({
        queryKey: ["attendance", "summaries", month, year],
      });
    },
    onError: (err: Error) =>
      toast.error(err.message || t("attendanceMonthlyClientGenericError")),
  });

  const lockMutation = useMutation({
    mutationFn: () => lockMonthlySummary({ month, year }),
    onSuccess: () => {
      toast.success(t("attendanceMonthlyClientLockSuccess"));
      queryClient.invalidateQueries({
        queryKey: ["attendance", "summaries", month, year],
      });
    },
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("attendanceMonthlyClientExportSuccess"));
    } catch (err) {
      toast.error(
        err instanceof Error
          ? err.message
          : t("attendanceMonthlyClientExportError"),
      );
    } finally {
  ...
  const handleLock = () => {
    if (
      !window.confirm(
        t("attendanceMonthlyClientLockConfirm", { month, year }),
      )
    )
      return;
    lockMutation.mutate();
