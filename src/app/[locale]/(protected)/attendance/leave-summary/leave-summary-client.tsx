...
export function LeaveSummaryClient({ initialData }: LeaveSummaryClientProps) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("leaveSummaryToastUpdated"));
    ...
    onError: (error: Error) => {
      toast.error(error.message || t("leaveSummaryToastError"));
    },
  });
    ...
    () => [
      {
        ...
        header: () => (
          <Checkbox
            ...
            aria-label={t("leaveSummarySelectAll")}
          />
        ),
        cell: ({ row }) => (
          <Checkbox
            ...
            aria-label={t("leaveSummarySelectRow", {
              name: row.original.user?.fullName || row.original.user?.name || "",
            })}
      ...
      },
      {
        ...
        header: t("leaveSummaryColEmployee"),
        cell: ({ row }) => {
          const name =
      ...
      },
      {
        ...
        header: t("leaveSummaryColDepartment"),
      ...
      },
      {
        ...
        header: t("leaveSummaryColLeaveType"),
        filterFn: (row, columnId, filterValue: string) => {
          if (filterValue === "all") return true;
      ...
      },
      {
        ...
        header: t("leaveSummaryColTotalDays"),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: t("leaveSummaryColUsedDays"),
        cell: ({ row }) => (
...

      {
        ...
        header: t("leaveSummaryColPendingDays"),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: t("leaveSummaryColAvailableDays"),
        cell: ({ row }) => {
          const available =
      ...
      {
        id: "actions",
        header: t("leaveSummaryColActions"),
        cell: ({ row }) => (
          <div className="flex items-center gap-1 justify-end">
            <Tooltip>
              ...
              <TooltipContent>{t("leaveSummaryActionEdit")}</TooltipContent>
            </Tooltip>
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    toast.success(t("leaveSummaryToastDeletedOne"));
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    toast.success(
      t("leaveSummaryToastDeletedMany", { count: batchDeleteTarget.length }),
    );
    queryClient.invalidateQueries({ queryKey: ["leave-balances"] });
  ...
  return (
    <div className="w-full min-h-0 h-full grow flex flex-col bg-background">
      <div className="w-full min-h-0 h-full min-w-0 flex flex-col relative">
        ...
        <section>
          <header className="p-2 flex items-center h-10 border-b justify-between">
            <h1 className="font-bold">{t("leaveSummaryTitle")}</h1>
            <Button
              variant={"outline"}
              size={"xs"}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative flex items-center" ref={mergedSearchRef}>
                <Input
                  ...
                  placeholder={t("leaveSummarySearchPlaceholder")}
                  className={cn(
          ...
          <TableSettingsPanel
            ...
            columnOptions={[
              {
                key: "employeeName",
                label: t("leaveSummaryColEmployee"),
                icon: User,
              },
              {
                key: "department",
                label: t("leaveSummaryColDepartment"),
                icon: BadgeCheck,
              },
              {
                key: "leaveTypeName",
                label: t("leaveSummaryColLeaveType"),
                icon: CalendarDays,
              },
              {
                key: "totalDays",
                label: t("leaveSummaryColTotalDays"),
                icon: BadgeCheck,
              },
              {
                key: "usedDays",
                label: t("leaveSummaryColUsedDays"),
                icon: BadgeCheck,
              },
              {
                key: "pendingDays",
                label: t("leaveSummaryColPendingDays"),
                icon: BadgeCheck,
              },
              {
                key: "availableDays",
                label: t("leaveSummaryColAvailableDays"),
                icon: BadgeCheck,
              },
            ]}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("leaveSummaryConfirmDeleteTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("leaveSummaryConfirmDeleteDescriptionPrefix")}{" "}
              <strong>
                {deleteTarget?.user?.fullName || deleteTarget?.user?.name}
              </strong>{" "}
              ({deleteTarget?.leaveType?.name}) {t("leaveSummaryConfirmDeleteDescriptionSuffix")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteTarget(null)}>
              {t("leaveSummaryCancel")}
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("leaveSummaryConfirmBatchDeleteTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("leaveSummaryConfirmBatchDeleteDescription", {
                count: batchDeleteTarget?.length ?? 0,
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {t("leaveSummaryDeleteMany", { count: batchDeleteTarget?.length ?? 0 })}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={editDialog} onOpenChange={setEditDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("leaveSummaryEditDialogTitle")}</DialogTitle>
          </DialogHeader>
          <Form {...editForm}>
            <form
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("leaveSummaryTotalDaysLabel")}</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={0}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("leaveSummaryUsedDaysLabel")}</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={0}
      ...
      <Dialog open={importDialog} onOpenChange={setImportDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("leaveSummaryImportDialogTitle")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {t("leaveSummaryImportDialogDescription")}
            </p>
            <Button variant="outline" className="w-full">
              <Upload className="h-4 w-4 mr-2" />
              {t("leaveSummarySelectExcelFile")}
            </Button>
          </div>
          <DialogFooter>
            <Button
