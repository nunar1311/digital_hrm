            ...
            {calendarData.sortedUsers.length === 0 ? (
              <div className="flex h-40 items-center justify-center text-muted-foreground">
                {debouncedSearch.trim()
                  ? t("attendanceShiftsNoResults")
                  : t("attendanceShiftsNoEmployees")}
              </div>
            ) : (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-13.5rem)] no-scrollbar">
            {calendarData.sortedUsers.length === 0 && (
              <div className="flex h-40 items-center justify-center text-muted-foreground">
                {debouncedSearch.trim()
                  ? t("attendanceShiftsNoMatchingEmployees", {
                      search: debouncedSearch,
                    })
                  : t("attendanceShiftsNoEmployeesOrAssignments")}
              </div>
            )}
            {calendarData.sortedUsers.map((user) => (
              <EmployeeRow
...
function DayHeader({
  ...
  const longDayNames = [
    t("attendanceShiftsWeekdaySunday"),
    t("attendanceShiftsWeekdayMonday"),
    t("attendanceShiftsWeekdayTuesday"),
    t("attendanceShiftsWeekdayWednesday"),
    t("attendanceShiftsWeekdayThursday"),
    t("attendanceShiftsWeekdayFriday"),
    t("attendanceShiftsWeekdaySaturday"),
  ];
  const dayOfWeek = isMonthView
    ? shortDayNames[day.getDay()]
    : longDayNames[day.getDay()];
    ...
    >
      ...
      {!isMonthView && (
        <span className="mt-0.5 text-[10px] text-muted-foreground">
          {t("attendanceShiftsDayShiftCount", { count: dayShiftCount })}
        </span>
      )}
    </div>
  );
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] text-muted-foreground">
            {totalHours > 0
              ? t("attendanceShiftsAssignedSummary", {
                  hours: totalHours.toFixed(0),
                  shifts: totalShifts,
                })
              : t("attendanceShiftsUnassigned")}
          </p>
        </div>
      </div>
            ...
            <Tabs defaultValue="shift" className="w-full">
              <TabsList className="grid w-full grid-cols-2 rounded-t-lg rounded-b-none">
                ...
                >
                  ...
                  <span>{t("attendanceShiftsChooseShift")}</span>
                </TabsTrigger>
                <TabsTrigger
                  value="cycle"
                  className="flex items-center gap-1.5"
                >
                  ...
                  <span>{t("attendanceShiftsAssignCycle")}</span>
                </TabsTrigger>
              </TabsList>

              {/* Tab 1: Shift selection */}
              <TabsContent value="shift" className="p-3">
                <p className="mb-2 text-xs font-medium text-muted-foreground">
                  {t("attendanceShiftsChooseShiftFor", { userName })}
                </p>
                <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
                  ...
                  {shifts.filter((s) => s.isActive).length === 0 && (
                    <p className="py-2 text-center text-xs text-muted-foreground">
                      {t("attendanceShiftsNoShifts")}
                    </p>
                  )}
                </div>
              </TabsContent>

              {/* Tab 2: Cycle assignment */}
