...
function ContractCard({ contract }: { contract: Contract }) {
    ...
    const isExpiringSoon = computedStatus === "ACTIVE" && contract.endDate && 
        (new Date(contract.endDate).getTime() - Date.now()) < 90 * 24 * 60 * 60 * 1000; // 90 days
            ...
            <CardContent className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        {contract.typeDuration && (
                            <p className="text-xs text-muted-foreground">
                                Thời hạn: {contract.typeDuration} tháng
                            </p>
                        )}
                    </div>
                    <div className="space-y-1">
                ...
                {isExpiringSoon && (
                    <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
                        ...
                        <div className="text-xs text-amber-800">
                            ...
                            <p>Vui lòng liên hệ bộ phận HCNS để gia hạn hoặc ký hợp đồng mới.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="flex items-center justify-between pt-3 border-t">
                    <div className="text-xs text-muted-foreground">
                        {contract.signedDate && (
                            <span>Đã ký ngày: {formatDate(contract.signedDate)}</span>
                        )}
                    </div>
                    {contract.fileUrl && (
...
export function ESSContractsClient({ initialContracts }: ESSContractsClientProps) {
    ...
    const stats = {
        ...
        expiringSoon: initialContracts.filter((c) => {
            ...
            return status === "ACTIVE" && c.endDate && 
                (new Date(c.endDate).getTime() - Date.now()) < 90 * 24 * 60 * 60 * 1000;
        }).length,
    };
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                                Xem thông tin hợp đồng và quyền lợi của bạn
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="text-sm">
                                ...
                                <ul className="mt-2 space-y-1 text-indigo-800 text-xs">
                                    ...
                                    <li>• Nếu có thắc mắc, liên hệ bộ phận HCNS để được giải đáp</li>
                                </ul>
                            </div>
                        </div>
                    </CardContent>
