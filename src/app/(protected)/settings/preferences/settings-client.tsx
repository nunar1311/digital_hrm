...
export function SettingsClient({
  ...
  const [flyoutToastEnabled, setFlyoutToastEnabled] = useState<boolean>(true);
  ...
  const [savedKeyboardShortcuts, setSavedKeyboardShortcuts] = useLocalStorage({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Không thể lưu cài đặt");
    } finally {
      setIsSaving(false);
    }
  }, [
  ...
  const timezone =
    settings[SYSTEM_FIELDS.timezone.key] ?? SYSTEM_FIELDS.timezone.default;
      ...
      <div className="flex-1 px-4 md:px-6 pb-6 overflow-auto h-full min-h-0 no-scrollbar">
        <div className="space-y-0">
          ...
          <SettingsLanguageRegion
            timezone={timezone}
            onTimezoneChange={(v) => updateField(SYSTEM_FIELDS.timezone.key, v)}
            canEdit={canEdit}
