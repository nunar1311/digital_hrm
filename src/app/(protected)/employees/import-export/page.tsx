...
import { FileUp, FileDown, Users, FileSpreadsheet, Info } from 'lucide-react';
...
import { validateImportedRows, type ParsedImportResult, type ValidationResult } from '@/lib/excel-utils';
...
export default function ImportExportPage() {
    ...
    const handleFileParsed = useCallback((result: ParsedImportResult) => {
        ...
        const validationResult = validateImportedRows(result.rows);
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            {/* Page Header */}
            <div>
                <h1 className="text-2xl font-bold tracking-tight">Nhập / Xuất Dữ liệu Nhân viên</h1>
                <p className="text-muted-foreground mt-1">
                    Nhập dữ liệu nhân viên từ file Excel hoặc xuất danh sách hiện tại ra file
                </p>
            </div>
                    ...
                    <Card>
                        <CardHeader className="pb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <CardTitle className="text-lg">Nhập dữ liệu</CardTitle>
                                    <CardDescription>Tải lên file Excel để nhập nhân viên mới</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent>
                            <FileDropzone onFileParsed={handleFileParsed} onClear={handleClear} />

                            {/* Format hint */}
                            <div className="mt-4 flex items-start gap-2 rounded-lg bg-muted/50 px-3 py-2.5">
                                <Info className="h-4 w-4 text-muted-foreground shrink-0 mt-0.5" />
                                <div className="text-xs text-muted-foreground space-y-1">
                                    <p><strong>Yêu cầu:</strong> File phải chứa cột <code className="bg-background px-1 rounded">Mã nhân viên</code> và <code className="bg-background px-1 rounded">Họ và tên</code></p>
                                    <p>Các cột khác: Ngày sinh, Giới tính, SĐT, Email, Phòng ban, Chức vụ, Ngày vào làm, Trạng thái</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <div className="flex flex-col gap-4">
                    <Card>
                        <CardHeader className="pb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <CardDescription>Tải xuống danh sách nhân viên dạng Excel</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="rounded-lg border bg-background p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">Cột xuất ra</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Columns included */}
                            <div>
                                <p className="text-sm font-medium mb-2">Các cột sẽ xuất:</p>
                                <div className="flex flex-wrap gap-1.5">
                                    {[
                                        'Mã NV', 'Họ tên', 'Ngày sinh', 'Giới tính', 'SĐT', 'Email',
                                        'Địa chỉ', 'Phòng ban', 'Chức vụ', 'Loại hình', 'Ngày vào',
                                        'Trạng thái', 'Ngân hàng', 'Số TK', 'Trình độ', 'Trường',
                                    ].map((col) => (
