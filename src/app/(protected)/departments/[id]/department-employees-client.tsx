            ...
            >
              ...
              >
                <p className="text-xs font-medium text-muted-foreground px-1 mb-1">
                  Chọn chức vụ cho {row.original.fullName || row.original.name}
                </p>
                <PositionDropdown
                  value={row.original.positionId}
                  departmentId={departmentId}
                  placeholder="-- Không chức vụ --"
                  ...
                  {!isLoadingEmployees && employees.length > 0 && (
                    <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-2 py-2 border-t shrink-0 bg-background">
                      <p className="text-xs text-muted-foreground">
                        Hiển thị <strong>{employees.length}</strong> /{" "}
            ...
            <AlertDialogDescription>
              ...
              {deletePositionTarget && deletePositionTarget.userCount > 0 ? (
                <span className="block mt-1 text-destructive">
                  Đang có <strong>{deletePositionTarget.userCount}</strong> nhân
                  viên đang giữ chức vụ này. Không thể xóa.
                </span>
              ) : (
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (result.success) {
                    ...
                    toast.error(result.error || "Lỗi khi xóa chức vụ");
                  }
                } catch {
