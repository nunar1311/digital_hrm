          ...
          <div className="flex items-center justify-between">
            ...
            {request.leaveType && (
              <Badge variant="outline" className="text-xs">
                {request.leaveType.isPaidLeave ? "Có lương" : "Không lương"}
              </Badge>
            )}
          </div>
...
export function LeaveRequestsClient({
  ...
  const [month, setMonth] = useState(0); // 0 = tất cả tháng
  ...
  // ─── Dialog ───
  const [detailRequest, setDetailRequest] = useState<LeaveRequestItem | null>(
    null,
  );
  ...
  const {
    ...
    placeholderData: initialRequests,
    staleTime: 0,
  });

  const { data: statsData = initialStats } = useQuery({
    queryKey: ["leave-request-stats", year, month],
    queryFn: () => getLeaveRequestStats(year, month || undefined),
    ...
    () => [
      {
        id: "select",
        header: () => {
          const selectedCount = tabData.filter((r) =>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge variant="secondary" className="text-[10px]">
            <DynamicIcon
              iconName={row.original.user.department?.logo || "Building2"}
            />
        ...
        cell: ({ row }) => (
          <div className="text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              <span>{formatDate(row.original.startDate, timezone)}</span>
            </div>
            <div className="flex items-center gap-1">
              <Calendar className="h-3 w-3" />
              <span>{formatDate(row.original.endDate, timezone)}</span>
            </div>
          </div>
        ),
      },
    ...
    <div className="w-full min-h-0 h-full grow flex flex-col bg-background">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-2 px-2 py-1.5 bg-primary/5 border-b border-primary/20 shrink-0">
          <span className="text-xs text-muted-foreground">
            Đã chọn{" "}
            <strong className="text-foreground">{selectedIds.size}</strong> đơn
          </span>
          <Button
            variant="default"
            size="xs"
      ...
      {!isLoading && requestsData.total > 0 && (
        <div className="shrink-0 px-2 py-2 border-t bg-background flex items-center justify-between">
          ...
          {statsData.pending > 0 && (
            ...
            >
              {statsData.pending} đơn chờ bạn duyệt
            </Badge>
          )}
        </div>
      )}
