...
export function MonthlyClient({
    ...
    const [viewMode, setViewMode] = useState<"grid" | "summary">(
        "grid",
    );
    ...
    const [visibleGridCount, setVisibleGridCount] =
        useState(PAGE_SIZE);
    const [visibleSummaryCount, setVisibleSummaryCount] =
        useState(PAGE_SIZE);

    // ─── Queries ───
    const { data, isFetching: isFetchingData } =
    ...
    const gridData = useMemo((): MonthlyGridRow[] => {
        ...
        return (users || []).map((user) => {
            ...
            const userSchedule =
                data.scheduledWorkDays?.[user.id] ?? [];
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                if (holidayDates.has(dateStr)) {
                    ...
                    const isScheduledToWork =
                        userSchedule.includes(d);
                    days.push(
                        att
    ...
    const loadMoreGrid = useCallback(() => {
        setVisibleGridCount((prev) =>
            Math.min(prev + PAGE_SIZE, gridData.length),
        );
    }, [gridData.length]);
