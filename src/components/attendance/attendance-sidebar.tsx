...
const AttendanceSidebar = () => {
  ...
  const handleCycleFormSubmit = async (
    ...
    const optimisticCycle = {
      ...
      isActive: editingWorkCycle ? editingWorkCycle.isActive : true,
      ...payload,
      entries: entries.map((e) => ({
        ...e,
        shift: e.shiftId ? shifts.find((s) => s.id === e.shiftId) : null,
      })),
    };
    ...
    const updater = (old: WorkCycle[]) => {
      ...
      if (editingWorkCycle) {
        return old.map((c: WorkCycle) =>
          c.id === editingWorkCycle.id ? optimisticCycle : c,
        );
      }
      return [...old, optimisticCycle];
    };

    queryClient.setQueryData(["attendance", "work-cycles-sidebar"], updater);
    queryClient.setQueryData(["attendance", "workCycles"], updater);
    ...
    try {
      ...
      if (previousCycles)
        queryClient.setQueryData(["attendance", "workCycles"], previousCycles);
    } finally {
  ...
  const [showInactiveCycles, setShowInactiveCycles] = useState(false);

  const [searchShifts, setSearchShifts] = useState("");
  const [searchShiftsExpanded, setSearchShiftsExpanded] = useState(false);

  const [searchCycles, setSearchCycles] = useState("");
  const [searchCyclesExpanded, setSearchCyclesExpanded] = useState(false);
  ...
  const handleSearchShifts = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (!searchShiftsExpanded) {
        ...
        setTimeout(() => searchShiftsRef.current?.focus(), 50);
      } else {
        if (searchShifts.trim()) {
          setSearchShifts("");
        }
  ...
  const filteredShifts = searchShifts.trim()
    ? displayShifts.filter(
        (s) =>
          s.name.toLowerCase().includes(searchShifts.toLowerCase()) ||
          s.code?.toLowerCase().includes(searchShifts.toLowerCase()),
      )
    : displayShifts;
  ...
  const filteredCycles = searchCycles.trim()
    ? displayCycles.filter((c) =>
        c.name.toLowerCase().includes(searchCycles.toLowerCase()),
      )
    : displayCycles;

  const cycleDialogOpen = createCycleDialogOpen || editCycleDialogOpen;
                  ...
                  <SidebarMenuItem className="relative">
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        !searchShiftsExpanded && "[&_svg]:hover:text-primary",
                      )}
                    >
                            ...
                            <ScrollArea className="max-h-[200px] overflow-y-auto no-scrollbar">
                              {filteredShifts.map((shift) => (
                                <ShiftItem
                                  key={shift.id}
                                  shift={shift}
                                  shiftColor={getShiftColor(shift.id)}
                  ...
                  <SidebarMenuItem className="relative">
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        "absolute right-1 top-1 rounded cursor-pointer hover:bg-accent transition-colors z-20",
                        !searchCyclesExpanded && "[&_svg]:hover:text-primary",
                      )}
                    >
                      <Search className="size-3.5!" />
                    </div>
                            ...
                            <ScrollArea className="max-h-[280px]">
                              {filteredCycles.map((cycle) => (
                                <WorkCycleItem
                                  key={cycle.id}
                                  workCycle={cycle}
                                  onEdit={() => handleEditWorkCycle(cycle)}
                                  onDelete={() => handleDeleteWorkCycle(cycle)}
                                />
                              ))}
                            </ScrollArea>
