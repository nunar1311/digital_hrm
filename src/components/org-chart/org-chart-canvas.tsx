...
function countEmployees(n: DepartmentNode): number {
  return (
    (n.employeeCount ?? 0) +
    n.children.reduce((s, c) => s + countEmployees(c), 0)
  );
}
...
export function OrgChartCanvas({
  ...
  const [zoom, setZoom] = useState<number>(
    isMobile ? DEFAULT_MOBILE : DEFAULT_ZOOM,
  );
  ...
  useEffect(() => {
    const handleMove = (e: MouseEvent) => {
      ...
      const { id, startX, startY, origOffset } = movingNodeRef.current;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (e.touches.length === 2 && lastTouchDistRef.current !== null) {
        ...
        setZoom((prev) =>
          Math.min(
            MAX_ZOOM,
            Math.max(MIN_ZOOM, +(prev + delta * ZOOM_STEP * 10).toFixed(2)),
          ),
        );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      setPan((prev) => {
        switch (e.key) {
          case "ArrowUp":
            return { ...prev, y: prev.y + PAN_STEP };
          case "ArrowDown":
            return { ...prev, y: prev.y - PAN_STEP };
          case "ArrowLeft":
            return { ...prev, x: prev.x + PAN_STEP };
          case "ArrowRight":
            return { ...prev, x: prev.x - PAN_STEP };
          default:
            return prev;
  ...
  const recalcAllLines = useCallback(() => {
    ...
    const actualZoom = containerRect.width / containerEl.offsetWidth || 1;

    // Helper: get unscaled coords relative to content container
    const rel = (rect: DOMRect) => ({
      cx: (rect.left + rect.width / 2 - containerRect.left) / actualZoom,
      ...
      cy: (rect.top + rect.height / 2 - containerRect.top) / actualZoom,
    });
    ...
    function collectHierarchy(nodes: DepartmentNode[]) {
      for (const n of nodes) {
        if (n.children.length > 0 && (expandedNodes.has(n.id) || !n.parentId)) {
    ...
    hPairs.forEach(({ parentId, childId }) => {
      const pEl = document.querySelector(`[data-dept-id="${parentId}"]`);
      const cEl = document.querySelector(`[data-dept-id="${childId}"]`);
      if (pEl && cEl) {
    ...
    function walkMatrix(nodes: DepartmentNode[]) {
      for (const n of nodes) {
        if (n.secondaryParentIds && n.secondaryParentIds.length > 0) {
    ...
    >
      ...
      >
        ...
        >
          ...
          {(hierarchyLines.length > 0 || matrixLines.length > 0) && (
              ...
              <svg className="absolute inset-0 w-full h-full overflow-visible">
                <defs>
                  ...
                  <marker
                    ...
                    orient="0"
                  >
                    <polygon points="0,0 10,0 5,6" className="fill-primary" />
                  </marker>
                </defs>
                {/* Hierarchy lines */}
          ...
          {hasData ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-muted-foreground">
              ...
              <p className="text-lg font-semibold">Chưa có dữ liệu phòng ban</p>
              <p className="text-sm mt-1">Hãy tạo phòng ban đầu tiên</p>
            </div>
          )}
        </div>
      </div>
      ...
      {hasData && !isMobile && (
        <div className="absolute bottom-4 left-4 text-[10px] text-muted-foreground bg-background/80 backdrop-blur-sm px-2.5 py-1 rounded-md shadow-sm">
          🖱️ Cuộn = zoom • Kéo = di chuyển • Click = chi tiết • Kéo thả nhân
          viên để chuyển phòng ban
        </div>
      )}
    </div>
  );
...
interface OrgChartBranchProps {
  ...
  onNodeMoveStart: (nodeId: string, clientX: number, clientY: number) => void;
  isRoot?: boolean;
