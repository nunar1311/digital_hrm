...
export function DepartmentDetailPanel({
  ...
  const filteredEmployees =
    department?.employees?.filter((emp) => {
        ...
        <SheetContent className="w-full max-w-full sm:max-w-md overflow-y-auto">
          <SheetHeader className="mb-4">
            <div className="flex items-center gap-2">
              ...
              <SheetTitle className="text-base">Chi tiết phòng ban</SheetTitle>
            </div>
          </SheetHeader>
            ...
            <div className="space-y-6 p-3">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      ...
                      <DropdownMenuItem
                        onClick={() =>
                          router.push(`/org-chart/${department.id}`)
                        }
                      >
              ...
              <section>
                <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                  <UserCircle className="h-4 w-4 text-primary" /> Trưởng phòng
                </h3>
                {department.manager ? (
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border">
                    ...
                    <Button
                      ...
                      className="h-8 w-8"
                      onClick={() =>
                        router.push(`/employees/${department.manager?.id}`)
                      }
                    >
              ...
              {department.employees && department.employees.length > 0 && (
                <section>
                  <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                    ...
                    Nhân viên ({department.employees.length})
                  </h3>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto no-scrollbar">
                    {filteredEmployees.map((emp) => (
                      <EmployeeListItem
                        ...
                        employee={emp}
                        showActions
                        onViewProfile={(id) => router.push(`/employees/${id}`)}
                      />
                    ))}
              ...
              {department.children.length > 0 && (
                <section>
                  <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                    <FolderTree className="h-4 w-4 text-primary" /> Phòng ban
                    trực thuộc ({department.children.length})
                  </h3>
                  <div className="space-y-2">
                    {department.children.map((child) => (
                      <button
                        ...
                        onClick={() => router.push(`/org-chart/${child.id}`)}
                      >
                        <span
