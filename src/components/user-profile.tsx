...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const hasSeenTour = localStorage.getItem("theme-customize-tour-completed");

    if (!hasSeenTour) {
      tour.start();
      tour.on("complete", () => {
        localStorage.setItem("theme-customize-tour-completed", "true");
      });
    }
      ...
      {isThemeOpen && (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            Tuỳ chỉnh chủ đề của bạn bằng cách thay đổi giao diện và màu sắc.
          </p>
          ...
          <DropdownMenuLabel className="p-0 text-xs">Chủ đề</DropdownMenuLabel>
          {/* Color theme */}
          <div className="mt-2">
            <ThemePreset />
          </div>
