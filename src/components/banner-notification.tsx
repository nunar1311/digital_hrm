...
async function fetchPreferences() {
    const { getNotificationSettings } = await import("@/app/(protected)/notifications/actions");
    return await getNotificationSettings();
}

async function updateBrowserPreference(enabled: boolean) {
    const { savePreferences } = await import("@/app/(protected)/notifications/actions");
...
export default function BannerNotification() {
    ...
    return (
        <div className="flex items-center justify-between flex-nowrap gap-4 bg-primary min-h-11 text-accent leading-5 py-2.5 px-4">
            <div className="grow flex items-center justify-between flex-wrap gap-2">
                ...
                <p className="grow-500 basis-[min-content] text-sm font-medium">
                    <strong>Digital HRM</strong> cần quyền truy cập để
                    gửi thông báo.
                </p>
                <Button
                    size="xs"
