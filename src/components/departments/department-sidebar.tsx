...
import { Search, X, Snowflake, Plus, ChevronsUpDown } from "lucide-react";
...
export const ExpandedStateContext = createContext<ExpandedStateContextValue>({
  expandedState: null,
  setExpandedState: () => {},
});
...
const DepartmentSidebar = ({ departmentTree }: DepartmentSidebarProps) => {
  const pathname = usePathname();
  const router = useRouter();
  const { settings } = useSettings();
  const [searchQuery, setSearchQuery] = useState("");
  const [expandedState, setExpandedState] = useState<ExpandedState>("all");
          ...
          <div className="flex items-center gap-0.5">
            <div className="flex items-center transition-all duration-150 transform-gpu translate-x-2 group-hover:translate-x-0 gap-0.5 ease-linear opacity-0 group-hover:opacity-100">
              <Button
                ...
                size="icon-xs"
                tooltip={
                  expandedState === "all" ? "Thu gọn tất cả" : "Mở rộng tất cả"
                }
                onClick={() =>
                  setExpandedState(expandedState === "all" ? "none" : "all")
                }
              ...
              >
                {departmentTree.map((item) => (
                  <DepartmentTree
                    ...
                    onEdit={handleEdit}
                    onAddChild={(id) => {
                      const d = departmentTree.find((n) => n.id === id);
                      if (d) handleEdit(d);
                    }}
