...
export function RoleDropdown({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        toast.error(result.error || "Lỗi khi cập nhật chức vụ");
      }
  ...
  return (
    <>
      ...
      s{/* Confirm dialog when promoting — affects existing HEAD or DEPUTY */}
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="flex flex-col">
              <span>
                Bạn có chắc muốn đặt <strong>{employeeName}</strong> làm{" "}
                {confirmTarget === "HEAD" ? "Trưởng phòng" : "Phó phòng"}?
              </span>
              {confirmTarget === "HEAD" ? (
                <span>
                  Nhân viên giữ vai trò <strong>Trưởng phòng</strong> hiện tại
                  (nếu có) sẽ bị hạ cấp thành <strong>Nhân viên</strong>.
                </span>
              ) : (
                <span>
                  Nhân viên giữ vai trò <strong>Phó phòng</strong> hiện tại (nếu
                  có) sẽ bị hạ cấp thành <strong>Nhân viên</strong>.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
