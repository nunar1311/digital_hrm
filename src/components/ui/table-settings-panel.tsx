...
export function TableSettingsPanel({
  ...
  const hasGeneralToggles =
    setShowEmptyDepartments !== undefined || setWrapText !== undefined;
    ...
    >
      <div className="flex flex-col h-full w-72">
        ...
        <div className="flex items-center justify-between px-4 py-3 shrink-0 border-b">
          <span className="text-sm font-semibold">Tùy chỉnh hiển thị</span>
          <Button
            size="icon-xs"
            variant="secondary"
          ...
          <div className="px-2 min-w-0 flex flex-col">
            {defaultVisibleColumns && (
              <span
                ...
                onClick={() => setColumnVisibility(defaultVisibleColumns)}
              >
                Ẩn tất cả cột
              </span>
            )}
            {columnOptions.map((option, index) => {
              ...
              const isHidden = hiddenColumnIndices.includes(index);
              return (
                <div
                  key={option.key}
                  className={cn(
                    ...
                    (isDisabled || isHidden) && "opacity-50 cursor-not-allowed",
                    isHidden && "hidden",
                  )}
