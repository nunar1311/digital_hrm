...
export function PayslipViewer({
    ...
    const earnings: PayslipItem[] = payslip.earnings
        ? JSON.parse(payslip.earnings).map((e: any) => ({ ...e, amount: Number(e.amount || 0) }))
        : [];
    const deductions: PayslipItem[] = payslip.deductions
        ? JSON.parse(payslip.deductions).map((d: any) => ({ ...d, amount: Number(d.amount || 0) }))
        : [];
    const insurance: PayslipInsurance = payslip.insurance
        ? (() => {
              const ins = JSON.parse(payslip.insurance);
              return { ...ins, BHXH: Number(ins.BHXH || 0), BHYT: Number(ins.BHYT || 0), BHTN: Number(ins.BHTN || 0) };
          })()
