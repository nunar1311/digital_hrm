...
const formulaSchema = z.object({
    ...
    code: z.string().min(1, "Mã công thức không được trống").regex(/^[A-Z0-9_]+$/, "Mã phải viết hoa, không dấu cách"),
...
export function FormulaEditor({ editData, onSave, onCancel }: FormulaEditorProps) {
    ...
    const evaluateExpression = useCallback((expr: string): number | string => {
        try {
            ...
            return typeof result === "number" && isFinite(result) ? result : "Kết quả không hợp lệ";
        } catch {
