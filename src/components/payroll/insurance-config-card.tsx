...
const insuranceConfigSchema = z.object({
    socialRate: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) >= 0, {
        message: "Tỷ lệ BHXH phải >= 0",
    }),
    healthRate: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) >= 0, {
        message: "Tỷ lệ BHYT phải >= 0",
    }),
    unemploymentRate: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) >= 0, {
        message: "Tỷ lệ BHTN phải >= 0",
    }),
    minSalary: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, {
        message: "Lương tối thiểu phải > 0",
    }),
    socialMaxSalary: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, {
        message: "Mức lương tối đa BHXH phải > 0",
    }),
    healthMaxSalary: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, {
        message: "Mức lương tối đa BHYT phải > 0",
    }),
    unemploymentMaxSalary: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, {
...
export function InsuranceConfigCard({
    ...
    const form = useForm<InsuranceConfigForm>({
        ...
        defaultValues: initialConfig
            ...
            : {
                  ...
                  unemploymentRate: String(DEFAULT_INSURANCE_RATES.unemploymentRate * 100),
    ...
    const handlePresetChange = (year: string) => {
        ...
        if (preset) {
            ...
            form.setValue("unemploymentRate", String(preset.rates.unemploymentRate * 100));
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Cấu hình tỷ lệ và mức lương tối đa đóng bảo hiểm xã hội, y tế, thất nghiệp
                </CardDescription>
            </CardHeader>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                <div className="rounded-lg bg-muted/50 p-4 space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <div className="flex justify-between">
                                            <span className="text-muted-foreground">Tổng tỷ lệ đóng:</span>
                                            <span className="font-medium">
                                                {(
                                        ...
                                        <div className="flex justify-between">
                                            <span className="text-muted-foreground">Mức đóng BHXH (lương 10M):</span>
                                            <span className="font-medium">
                                                {formatCurrency(
                                                    Math.min(parseFloat(values.minSalary || "0") * 20, parseFloat(values.socialMaxSalary || "0")) *
