...
import { exportContractWithAI } from "@/app/(protected)/contracts/ai-actions";
...
export function ContractDetailClient({ contractId, initialData }: Props) {
  ...
  const exportAIMutation = useMutation({
    mutationFn: async ({ contractId }: { contractId: string }) => {
      return exportContractWithAI({ contractId });
    },
    onSuccess: (result) => {
      if (
        !result.success ||
        ...
        toast.error(result.error || "Không thể xuất file DOCX từ AI");
        return;
      }
      const blob = base64ToBlob(result.base64Content, result.mimeType);
      const url = URL.createObjectURL(blob);
      ...
      document.body.removeChild(anchor);
      URL.revokeObjectURL(url);
      toast.success("Đã xuất file hợp đồng bằng AI.");
    },
    onError: () => {
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {data.employee.name} ({data.employee.username || "N/A"})
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button
              variant="default"
              onClick={() => exportAIMutation.mutate({ contractId })}
